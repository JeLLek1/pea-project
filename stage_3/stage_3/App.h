...
public:
	...
	static unsigned int cinUInt(unsigned int min);
	// wczytanie liczby typu float z konsoli
	static float cinFloat(float min, float max);
	// zamiana dwóch elementów vector
	static void swap(std::vector<size_t>* vec, size_t fisrt, size_t second);
};
